...
class InternalCommandDoctor extends InternalCommand {
  ...
  Future<void> run(final List<String> args, Context context) async {
    ...
    final checks = [
      ...
      _checkUserEmail(context),
      _checkRemote(context, isInsideWorkTree),
      _checkDefaultBranch(context, isInsideWorkTree),
      _checkGh(context, isInsideWorkTree),
    ];
  ...
  String _boolToCheckmark(bool value) => value ? 'V' : 'X';

  Future<String> _checkUserName(Context context) async {
    final userName =
        (await context
                .quietly()
                .git
    ...
    final result = StringBuffer(
      '''[${_boolToCheckmark(hasUserName)}] git config --get user.name # $userName''',
    );
  ...
  Future<String> _checkUserEmail(Context context) async {
    final userEmail =
        (await context
                .quietly()
                .git
    ...
    final result = StringBuffer(
      '''[${_boolToCheckmark(hasUserEmail)}] git config --get user.email # $userEmail''',
    );
  ...
  Future<String?> _checkRemote(Context context, bool isInsideWorkTree) async {
    if (!isInsideWorkTree) return null;

    final remote = context.getPreferredRemote();
    final hasRemote = remote != null;
    final result = StringBuffer(
      """[${_boolToCheckmark(hasRemote)}] git remote # ${hasRemote ? "remote(s): $remote" : "no remotes"}""",
    );
  ...
  Future<String?> _checkDefaultBranch(
    Context context,
    ...
    final result = StringBuffer(
      """[${_boolToCheckmark(defaultBranch != null)}] git rev-parse --abbrev-ref $remote/HEAD # ${defaultBranch ?? "not found"}""",
    );
  ...
  Future<String> _checkGh(Context context, bool isInsideWorkTree) async {
    String? ghVersion;
    try {
      ghVersion =
          (await context
    ...
    final result = StringBuffer(
      '''[${_boolToCheckmark(ghVersion?.isNotEmpty == true)}] gh --version # $ghVersion''',
    );
    ...
    result.write(
      '''\n[${_boolToCheckmark(isAuthenticated)}] gh auth status # ${isAuthenticated ? "authenticated" : "not authenticated"}''',
    );
    ...
    if (isInsideWorkTree) {
      ...
      result.write(
        '''\n[${_boolToCheckmark(canAccessRepo)}] gh repo view # ${canAccessRepo ? "has access" : "no access"}''',
      );
