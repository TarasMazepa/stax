...
import 'package:stax/context/context_rebase_settings.dart';
  ...
  void run(List<String> args, Context context) {
    ...
    if (context.rebaseSettings != null) {
      ...
      final rebaseData = RebaseData(
        ...
        steps: branches.map((branch) => RebaseStep(data: branch)).toList(),
      );

      context.rebaseSettings!.rebaseData.value = rebaseData;
      context.printToConsole('Saved rebase data to rebase settings.');
    }
    ...
    if (context.rebaseSettings != null) {
      context.rebaseSettings!.rebaseData.clear();
      context.printToConsole('Cleared rebase data from rebase settings.');
    }
  }
}
