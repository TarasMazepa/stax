...
typedef HttpResponseHead = ({int statusCode, Map<String, String> headers});

extension HttpResponseHeadExtension on HttpResponseHead {
...
class HttpResponseReader {
  ...
  Future<HttpResponseHead> readHead() async {
    ...
    while (true) {
      ...
      if (_done) {
        ...
        throw Exception('Connection closed before HTTP headers');
      }
      await _waitForData();
    }
  }
  ...
  Future<List<int>> _readExactly(int n) async {
    while (_buffer.length < n) {
      if (_done) {
        if (_error != null) throw _error!;
        throw Exception('Connection closed before $n body bytes');
      }
      await _waitForData();
    }
  ...
  Future<String> _readLine() async {
    ...
    while (true) {
      ...
      if (_done) {
        if (_error != null) throw _error!;
        throw Exception('Connection closed mid-line');
      }
      await _waitForData();
    }
  }
